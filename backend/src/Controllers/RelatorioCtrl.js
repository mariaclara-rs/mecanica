...
module.exports = {
    ...
    async CaixaPeriodoSimplificado(request,response){
        const {dtIni, dtFim} = request.query;
        const sql = "select * from ((select os.os_dataFechamento as data, 'OS' as tipo, sum(os.os_valTot - os.os_valFiado) as val "
        +"from ordemservico os "
        +"where "
        +"os.os_valTot > os.os_valFiado "
        +"group by os.os_id) "
       	+"union "
        +"(select cr.cr_dtReceb, 'CR', cr.cr_valorReceb "
        +"from contareceber cr "
        +"where "
        +"cr.cr_valorReceb > 0) "
        +"union "
        +"(select p.parc_dtPgto ,'CP', sum(p.parc_val) from parcela p "
        +"where "
        +"p.parc_val>0 "
        +"group by p.cp_id, p.parc_dtPgto)) as tab "
        +"where tab.data between ? and ? "
